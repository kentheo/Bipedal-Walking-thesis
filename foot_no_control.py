...
if __name__=="__main__":
    ...
    while True:
        ...
        # raw_data[2][0] = s[8]  # Ankle1 angle
        # raw_data[2][1] = s[9]  # Ankle1 vel
        # raw_data[3][0] = s[11]  # Hip2 angle
        # raw_data[3][1] = s[12] # Hip2 vel
        # raw_data[4][0] = s[13] # Knee2 angle
        # raw_data[4][1] = s[14] # Knee2 vel
        # raw_data[5][0] = s[15]  # Ankle2 angle
        # raw_data[5][1] = s[16]  # Ankle2 vel
        # raw_data[6][0] = s[0]  # hull angle
        # raw_data[6][1] = s[1]  # hull vel
        ...
        # current_hip_targ = [hip_targ[0] - s[4], hip_targ[1] - s[11]]
        ...
        # hip_todo[1] = 1800.0*(hip_targ[1] - s[11]) - 0.1*s[10]  # hip angle/hip speed leg 2

        hip_todo[0] = 100.5*(hip_targ[0] - s[4]) - 0.85*x_ft[0][1] #s[5]  # hip angle/hip speed leg 1
        hip_todo[1] = 100.5*(hip_targ[1] - s[11]) - 0.85*x_ft[3][1] #s[10] # hip angle/hip speed leg 2
        ...
        # Previous knee hack
        # if s[0]>0.0:
        #     knee_todo[supporting_leg] += 200*(h_c - s[18])- 20.0*s[3]
        # if s[0]<0.0:
        #     knee_todo[supporting_leg] -= 200 * (h_c - s[18]) + 20.0 * s[3]
        ...
        ankle_ang_filt.append(x_ft[2][0])

        a[0] = hip_todo[0]
        a[1] = knee_todo[0]
        a[2] = 0.0#ankle_todo[0]
        a[3] = hip_todo[1]
        a[4] = knee_todo[1]
        a[5] = 0.0#ankle_todo[1]
    ...
    if plot_flag == "ankles":
        ...
        plt.plot(t_list, torque_a2, 'r')
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("ankle torque", fontsize=10)
        fig1.show()
        fig2 = plt.figure(2)
        plt.plot(t_list, ankle_ang_real, 'r', t_list, ankle_ang_filt, 'g')
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Ankle angles real and filtered", fontsize=10)
        fig2.show()
    #vel_filter = []
    #alpha = 10.0
