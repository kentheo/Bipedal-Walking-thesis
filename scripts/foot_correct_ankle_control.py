...
x_i = 5.2/SCALE   # initial position of CoM wrt the standing point
foot_h = LEG_W/2
...
if __name__=="__main__":
    ...
    while True:
        ...
        # IK for ankle
        beta_mv_est = math.pi/2 - (math.pi - (b2 + (math.pi/2 - b1)))
        ...
        angularVelocity_d = 1.5 * (-0.1 - actual_angle_body) - 0.1 * angular_velocity_body
        ...
        # PD Controllers
        hip_todo[0] = 48.5*(hip_targ[0] - s[4]) - 0.85*x_ft[0][1] #s[5]  # hip angle/hip speed leg 1
        hip_todo[1] = 48.5*(hip_targ[1] - s[11]) - 0.85*x_ft[3][1] #s[10] # hip angle/hip speed leg 2
        ...
        # Ankle angles graphs
        ankle_ang_real.append(s[15])
        ankle_ang_targ.append(-1*ankle_targ[1])
    ...
    if plot_flag=="hip":
        ...
        plt.ylabel("Desired knee angle of leg 2 (rad)", fontsize=12)
    ...
    if plot_flag=="two":
        ...
        plt.xlabel("time (s)", fontsize=12)
        plt.ylabel("Joint angles of Leg 1 (rad)", fontsize=12)
    ...
    if plot_flag=="height":
        ...
        plt.xlabel("time (s)", fontsize=12)
        plt.ylabel("Height of CoM (m)", fontsize=12)
        plt.legend(loc="lower center", fontsize=12)
        plt.grid()
    ...
    if plot_flag=="all":
        fig1 = plt.figure(1)
        red, = plt.plot(t_list, gamma_real_0, 'r')
        green, = plt.plot(t_list, gamma_targ_0, 'g')
        plt.xlabel("Time (s)", fontsize=12)
        plt.ylabel("Hip angle (rad)", fontsize=12)
        plt.legend([red, (red, green)], ["Actual", "Target"])
        fig1.show()
        fig2 = plt.figure(2)
        red, = plt.plot(t_list, knee_real_0, 'r')
        green, = plt.plot(t_list, knee_targ_0, 'g')
        plt.xlabel("Time (s)", fontsize=12)
        ...
        fig3.show()
        fig4 = plt.figure(4)
        red, = plt.plot(t_list, hull_real, 'r')
        green, = plt.plot(t_list, hull_targ, 'g')
        plt.xlabel("Time (s)", fontsize=12)
        plt.ylabel("Hull angle (rad)", fontsize=12)
        plt.legend([red, (red, green)], ["Actual", "Target"])
        fig4.show()
        fig6 = plt.figure(6)
        red, = plt.plot(t_list, ankle_ang_real, 'r')
