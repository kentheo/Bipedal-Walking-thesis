...
# IK for support leg
def geometry_sp(x):
    L = LEG_H     # half the leg (one side of isosceles triangle) ??? half doesn't work
    ...
    gamma = help_angle - math.atan(x/h_c)
    # print("theta: {:f}, gamma: {:f}".format(theta, gamma))
    return theta, gamma

theta_i, gamma_i = geometry_sp(x_i)
...
if __name__=="__main__":
    ...
    x_ft = np.zeros((5,2))
    raw_data = np.zeros((5,2))


    while True:
        t = (steps)/float(FPS)

        # Estimate CoM position and velocity by LIPM
        x_t_est = x_s*math.cosh(t/T_c) + T_c*v_s*math.sinh(t/T_c)
        v_t_est = x_s/T_c*math.sinh(t/T_c) + v_s*math.cosh(t/T_c)
        
        # Calculations for Foot Placement
        ...
        # IK to obtain hip (gamma) and knee (theta) angles of supporting leg
        ...
        #print s[14]
        # print("Starting position of body: " + str(s[0]) + ", at: " + str(steps))
        ...
        # helper = ((2*LEG_H**2) - virtual_leg**2) / (2*LEG_H**2)
        # # Knee angle
        # theta_mv_est = math.acos(helper)
        # helper2 = (math.pi - theta_mv_est) / 2
        # # Hip angle
        # gamma_mv_est = helper2 + math.atan(((d+x_t_est-x_s)-(x_ankle_t-x_ankle_s))/(h_c-z_ankle_t))
        # print("theta_mv_est: {:f}, gamma_mv_est: {:f}".format(theta_mv_est, gamma_mv_est))
        virtual_leg = math.sqrt((h_c-z_ankle_t)**2+((d+x_t_est-x_s)-(x_ankle_t-x_ankle_s))**2)
        ...
        # Create the raw data matrix: hip, knee, hip, knee, body
        ...
        # raw_data[4][0] = s[0]  # hull angle
        # raw_data[4][1] = s[1]  # hull angular vel
        ...
        K_p = 1.5
        K_d = 0.13
        actual_angle_body = x_ft[4][0] #s[0]
        angular_velocity_body = x_ft[4][1] #s[1]
        ...
        # print("Accumulated delta_hip_angle: " + str(delta_hip_angle) + " at supporting_leg: " + str(supporting_leg) + " at stage: " + str(stage))

        hip_targ[supporting_leg] -= delta_hip_angle

        # if actual_angle_body > 0.0:
        #     hip_targ[supporting_leg] -= delta_hip_angle
        # else:
        #     hip_targ[supporting_leg] += delta_hip_angle

        ############################################
        # Removed the filter from the position and left for speed
        ############################################

        # PD Controllers
        # hip_todo[0] = 45*(hip_targ[0] - x_ft[0][0]) - 0.85*x_ft[0][1] #s[5]  # hip angle/hip speed leg 1
        # hip_todo[1] = 45*(hip_targ[1] - x_ft[2][0]) - 0.85*x_ft[2][1] #s[10] # hip angle/hip speed leg 2
        ...
        supp_leg_args = [0.0, 0.0]
        mov_leg_args = [0.0, 0.0]
        if moving_leg == 0:
            mov_leg_args = [s[4], x_ft[0][1]]
            supp_leg_args = [s[9], x_ft[2][1]]
        else:
            mov_leg_args = [s[9], x_ft[2][1]]
            supp_leg_args = [s[4], x_ft[0][1]]

        hip_todo[supporting_leg] = 45.5*(hip_targ[supporting_leg] - supp_leg_args[0]) - 0.85*supp_leg_args[1] #s[5]  # hip angle/hip speed leg 1
        hip_todo[moving_leg] = 22.5*(hip_targ[moving_leg] - mov_leg_args[0]) - 0.85*mov_leg_args[1]
        ...
        # hip_todo[0] -= 20.5*(0-s[0]) - 0.0*s[1]
        # hip_todo[1] -= 20.5*(0-s[0]) - 0.0*s[1]

        # knee_todo[0] = 21.0 * (knee_targ[0] - x_ft[1][0]) - 5.3 * x_ft[1][1]#s[7]     # knee angle/knee speed leg 1
        # knee_todo[1] = 21.0 * (knee_targ[1] - x_ft[3][0]) - 5.3 * x_ft[3][1]#s[12]   # knee angle/knee speed leg 2
        # knee_todo[0] = 260.0 * (knee_targ[0] - s[6]) - 4.0 * s[7]     # knee angle/knee speed leg 1
        # knee_todo[1] = 260.0 * (knee_targ[1] - s[11]) - 4.0 * s[12]   # knee angle/knee speed leg 2

        knee_todo[0] = 200.0 * (knee_targ[0] - s[6]) - 4.0 * x_ft[1][1]#s[7]     # knee angle/knee speed leg 1
        knee_todo[1] = 200.0 * (knee_targ[1] - s[11]) - 4.0 * x_ft[3][1]#s[12]   # knee angle/knee speed leg 2


        # Previous Height Control
        # if s[0]>0.0:
        #     knee_todo[supporting_leg] += 200*(h_c - s[14])- 4.0*s[3]
        # if s[0]<0.0:
        #     knee_todo[supporting_leg] -= 200 * (h_c - s[14]) + 4.0 * s[3]
    ...
    if plot_flag=="all":
        ...
        plt.ylabel("Hip angles", fontsize=10)
        fig1.show()
        fig2 = plt.figure(2)
        plt.plot(t_list, knee_targ_0, 'g', t_list, knee_real_0, 'r')
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Knee angles", fontsize=10)
        fig2.show()
        fig3 = plt.figure(3)
        ...
        plt.plot(t_list, hull_real, 'r', t_list, hull_targ, 'g')
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("hull angle", fontsize=10)
        fig4.show()
