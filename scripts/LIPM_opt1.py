...
if __name__=="__main__":
    ...
    while True:
        ...
        # PD Controllers
        hip_todo[0] = 1800.0*(hip_targ[0] - s[4]) - 0.1*s[5]
        hip_todo[1] = 1800.0*(hip_targ[1] - s[9]) - 0.1*s[10]
        hip_todo[0] -= 1800.0*(0-s[0]) - 0.0*s[1]
        hip_todo[1] -= 1800.0*(0-s[0]) - 0.0*s[1]

        knee_todo[0] = 260.0 * (knee_targ[0] - s[6]) - 4.0 * s[7]
        knee_todo[1] = 260.0 * (knee_targ[1] - s[11]) - 4.0 * s[12]
        ...
        if t >= T_step:
            ...
            # if env.legs[1].ground_contact:
            #     print('**x_s: {:3f}, foot position: {:3f}'.format(x_s, s[15]))
            # elif env.legs[3].ground_contact:
            #     print('//x_s: {:3f}, foot position: {:3f}'.format(x_s, s[16]))
