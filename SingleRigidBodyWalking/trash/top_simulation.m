...
% r = 0.025; a = 0.6; c = 0.8;           %  radius, height/thicknes, c is center of mass position with respect to contact point [m]
% MakeTop(1, r,a,c);
id=1;
wdh=[0.05, 0.4, 0.8 ];
mass=30;
...
v_des=0;
uLINK(1).R = Rodrigues([0 1 0],deg2rad(-17));   % orientation of rigid body
uLINK(1).w_local = [0 (vx_des-v_des)/norm(uLINK(1).c) 0]';  % angular velocity of the rigid body in local frame
% uLINK(1).w_local = [0 0 0]';  % angular velocity of the rigid body in local frame
...
AX=[-0.5 1.5];  AY=[-.1 .5]; AZ=[-.05 1.3];  % 3D plot dimension
...
for n = 1:length(time)
    uLINK(1).com = uLINK(1).R * uLINK(1).c + uLINK(1).p;  % needed for gravity torque
    uLINK(1).v = uLINK(1).vo + cross(uLINK(1).w,uLINK(1).p);    % needed for friction force
    [f,fc,tau] = contactForce(1);                           %  simulate contact forces
