    ...
    def _reset(self):
        ...
        for i in [-1,+1]:
            ...
            rjd = revoluteJointDef(
                ...
                lowerAngle = -1.4,#0.35,#3.1399,    # 20 degree
                upperAngle = 1.4,#3.14,#3.1401,    # 180 degree
                )
            ...
            #     angle = 0.0,
            #     fixtures = fixtureDef(
            #         shape=polygonShape(box=(LEG_W/4, LEG_H/4)),
            #         density=1.0,
            #         restitution=0.0,
            #         categoryBits=0x0020,
            #         maskBits=0x001)
            ...
            #     bodyA=lower,
            #     bodyB=foot,
            #     localAnchorA=(-LEG_W/2, -LEG_H/3),
            #     localAnchorB=(-LEG_W/2, LEG_H/3),
            #     enableMotor=False,
            #     enableLimit=False,
            #     maxMotorTorque=0.0,
            #     motorSpeed = 0.0,
...
if __name__=="__main__":
    ...
    while True:
        ...
        # PD Controllers
        hip_todo[0] = 4.5*(hip_targ[0] - x_ft[0][0]) - 0.85*x_ft[0][1] #s[5]  # hip angle/hip speed leg 1
        hip_todo[1] = 4.5*(hip_targ[1] - x_ft[2][0]) - 0.85*x_ft[2][1] #s[10] # hip angle/hip speed leg 2
    ...
    if plot_flag == "hip filters":
        ...
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Hip1 angular velocity", fontsize=10)
        ...
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Hip2 angular velocity", fontsize=10)
        plt.legend([red, (red, green)], ["Raw", "Filtered"])
        fig2.show()
        ...
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Hip to do 1(torque)", fontsize=10)
        fig5.show()
        ...
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("hip angle", fontsize=10)
        ...
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Hip1 actual angle and filter", fontsize=10)
        plt.legend([red, (red, green)], ["Raw", "Filtered"])
        fig7.show()
    if plot_flag == "knee filters":
        ...
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Knee1 angular velocity", fontsize=10)
        plt.legend([red, (red, green)], ["Raw", "Filtered"])
        fig1.show()
        ...
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("Knee2 angular velocity", fontsize=10)
        plt.legend([red, (red, green)], ["Raw", "Filtered"])
        fig2.show()
        ...
        green, = plt.plot(t_list, hip_targ_plot, 'g^')
        plt.xlabel("time (s)", fontsize=10)
        plt.ylabel("knee angle", fontsize=10)
        plt.legend([red, (red, green)], ["Actual", "Target"])
        fig6.show()
    #vel_filter = []
    #alpha = 10.0
